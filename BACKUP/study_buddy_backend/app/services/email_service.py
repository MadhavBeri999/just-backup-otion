...
def send_success_email(
    ...
    try:
        ...
        current_time = (datetime.utcnow() + timedelta(hours=5, minutes=30)).strftime(
            "%I:%M %p IST on %d %b %Y"
        )
...
"""
        ...
        if report_content:
            msg.add_attachment(
                ...
                maintype="text",
                subtype="plain",
                filename=f"Report_{child_name.replace(' ', '_')}.txt",
            )
        ...
        print(
            f"✅ [email_service] Success Email dispatched successfully to {parent_email}"
        )
...
"""
        ...
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
            ...
            server.send_message(msg)
        print(
            f"✅ [email_service] Failure Alert Email dispatched successfully to {parent_email}"
        )
    except Exception as e:

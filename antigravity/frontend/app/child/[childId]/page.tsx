            ...
            setRemainingTime((prev) => {
                ...
                if (prev <= 1) {
                    ...
                    if (token && activeSessionId) {
                        fetch(`http://localhost:8001/sessions/${activeSessionId}/end`, {
                            method: "POST",
                            headers: { Authorization: `Bearer ${token}` }
                        }).then(() => {
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                ...
                                <button
                                    onClick={async () => {
                                        ...
                                        await fetch(`http://localhost:8001/sessions/${activeSessionId}/end?status=terminated${latLngStr}`, {
                                            method: "POST",
                                            headers: { Authorization: `Bearer ${token}` },
                                        })
                                        window.location.href = `/child/${childId}`
                        ...
                        {tasks.map((task, index) => (
                            ...
                            >
                                <TaskCard
                                    ...
                                    hideStartGrinding={task.is_completed === 1}
                                />
                            </motion.div>
                        ))}
                    </AnimatePresence>

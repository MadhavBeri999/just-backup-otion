...
export default function StudySessionPage() {
  ...
  const endSessionAPI = async () => {
    const token = localStorage.getItem("token")
    if (token) {
      fetch(`http://localhost:8001/sessions/${sessionId}/end`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      }).catch(() => { })
    }
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (!gameState.current.isComplete && !gameState.current.isGameOver) {
        ...
        // 1. Play animation and sound!
        triggerDistraction("cross-button");

        // 2. Set Custom Defeat Text
  ...
  return (
    <ChalkboardLayout>
      ...
      <AnimatePresence>
        {sessionComplete && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative glass-strong rounded-3xl p-10 max-w-lg w-full text-center border border-white/10">
      ...
      <ExitTrapModal
        ...
        onConfirmExit={() => { window.location.href = "/dashboard"; }}
      />
    </ChalkboardLayout>
  )
}

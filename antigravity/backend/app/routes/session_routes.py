...
def end_session(
    ...
    if child and current_parent.email and not getattr(session, "parent_notified", False):
...
def get_remaining_time(
    ...
    if session.status != "active":
        return {"remaining_seconds": 0, "task_title": session.task.title, "task_id": session.task_id}

    task_duration = session.task.duration_minutes
    expected_end = session.start_time + timedelta(minutes=task_duration)
    remaining = (expected_end - datetime.utcnow()).total_seconds()

    return {"remaining_seconds": max(0, int(remaining)), "task_title": session.task.title, "task_id": session.task_id}
